...
public class ProductRecommenderUtility{
	...
	public HashMap<String,String> readOutputFile(){
		...
		try {
			...
			br = new BufferedReader(new FileReader(new File(TOMCAT_HOME+"\\webapps\\Grocery Hub\\Python\\matrixFactorizationBasedRecommendations.csv")));
		...
		// System.out.println("The HashMap is this:"+prodRecmMap);
		return prodRecmMap;
	}
	...
	public static Product getProduct(String product){
		...
		try 
		{
			...
			String selectProd="select * from  product_details where ProductName=?";
			...
			while(rs.next())
			{	
				prodObj = new Product(rs.getString("ProductId"),rs.getString("ProductName"),rs.getDouble("Price"),rs.getString("Image"));
			}
			rs.close();
			pst.close();
			conn.close();
